...
- 결정 결과를 `/project/state/decision_log.md`에 기록한다.
...
Epoch 1: propose(1) → challenge(1) → revise(1) = 3 sessions_send 호출
  ↓ 미수렴 시
[요약 작성 → /project/state/decision_log.md 저장]
  ↓
...
2. 요약을 `/project/state/decision_log.md`에 append한다.
...
- 강제 결정 시 `/project/state/decision_log.md`에 `[TIE-BREAK]` 태그를 붙인다.
...
- 해시를 `/project/state/debate_hashes.json`에 저장
...
- 해시가 `/project/state/debate_hashes.json`에 이미 존재하면 stale debate로 판정
...
| 파일 | 경로 | 용도 |
|------|------|------|
| Decision Log | `/project/state/decision_log.md` | 각 epoch 요약 및 최종 결정 기록 |
| Debate Hashes | `/project/state/debate_hashes.json` | 루프 감지용 해시 저장 |
| Run State | `/project/state/run_state.json` | 시스템 실행 상태 |
