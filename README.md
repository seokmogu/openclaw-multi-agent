...
| **Python 3.8+** | 상태 관리 및 비용 추적 스크립트 실행 | `brew install python` |
...
시스템은 설정된 예산 범위 내에서 작동합니다. 다음 파이썬 코드를 통해 현재 비용 현황을 확인할 수 있습니다.
...
print(f'Hourly Budget: ${ledger["hourly_budget_usd"]:.2f}')
```
...
├── tools/
│   └── cli/                            # CLI 래퍼 스크립트
│       ├── common.sh                   # 공통 함수 (예산 검사, 타임아웃, 비용 기록)
